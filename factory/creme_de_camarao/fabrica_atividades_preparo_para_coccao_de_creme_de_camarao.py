...
from models.atividades.subproduto.creme_de_camarao.preparo_para_coccao_de_creme_de_camarao import (
    PreparoParaCoccaoDeCremeDeCamarao,
)
...
# ============================================
logger = setup_logger(
    "SimulacaoPreparoCoccaoCremeDeCamarao",
    arquivo="logs/simulacao_preparo_coccao_creme_de_camarao.log"
)
...
# ============================================
quantidades = [5000, 15000, 25000, 50000, 60000, 5000, 10000, 8700]
...
for i, quantidade in enumerate(quantidades):
    atividade = PreparoParaCoccaoDeCremeDeCamarao(
        id=i + 1,
        tipo_atividade=TipoAtividade.PREPARO_PARA_COCCAO_DE_CREME_DE_CAMARAO,
        tipos_profissionais_permitidos=[TipoProfissional.COZINHEIRO],
        quantidade_funcionarios=1,
        equipamentos_elegiveis=[bancada_7],
        quantidade_produto=quantidade,
        fips_equipamentos={bancada_7: 1},  # fator de importancia padrão
    )
    atividades.append(atividade)

logger.info(
    f"🛠️ {len(atividades)} atividades de preparo para cocção de creme de camarão criadas."
)
