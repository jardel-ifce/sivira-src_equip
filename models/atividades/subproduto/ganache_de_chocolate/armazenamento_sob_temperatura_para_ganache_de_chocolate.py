...
logger = setup_logger('AtividadeArmazenamentoGanacheDeChocolate')


class ArmazenamentoSobTemperaturaParaGanacheDeChocolate(Atividade):
    """
    🍫 Atividade de armazenamento da ganache de chocolate em câmara refrigerada a 4°C.
    ✅ Ocupação feita em caixas de 30kg (mínimo 1 caixa, máximo 200).
    ...
    def calcular_duracao(self):
        ...
        if 3000 <= q <= 10000:
            ...
            logger.error(
                f"❌ Quantidade {q} fora das faixas válidas para armazenamento da ganache de chocolate."
            )
            raise ValueError(
                f"❌ Quantidade {q} fora das faixas válidas para armazenamento da ganache de chocolate."
            )
