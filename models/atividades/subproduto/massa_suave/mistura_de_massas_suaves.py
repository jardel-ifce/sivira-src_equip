...
logger = setup_logger('Atividade_Mistura_Massas_Suaves')


class MisturaDeMassasSuaves(Atividade):
    """
    🌀 Atividade que representa a mistura de massas suaves.
    ...
    def calcular_duracao(self):
        ...
        if 3000 <= q <= 17000:
            ...
            logger.error(
                f"❌ Quantidade {q} fora das faixas válidas para MISTURA DE MASSAS SUAVES."
            )
            raise ValueError(
                f"❌ Quantidade {q} fora das faixas válidas para MISTURA DE MASSAS SUAVES."
            )
        ...
        logger.info(
            f"🕒 Duração calculada: {self.duracao} para {q}g de massa suave."
        )
