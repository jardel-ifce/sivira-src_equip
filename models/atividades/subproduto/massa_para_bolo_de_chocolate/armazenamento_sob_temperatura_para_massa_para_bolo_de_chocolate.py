...
# 🔥 Logger específico
logger = setup_logger('AtividadeArmazenamentoMassaBoloChocolate')


class ArmazenamentoSobTemperaturaParaMassaParaBoloDeChocolate(Atividade):
    """
    🍫 Atividade de armazenamento da massa para bolo de chocolate em câmara refrigerada a 4°C.
    ...
    def calcular_duracao(self):
        ...
        if 3000 <= q <= 6000:
            ...
            logger.error(
                f"❌ Quantidade {q} fora das faixas válidas para armazenamento da massa de bolo de chocolate."
            )
            raise ValueError(
                f"❌ Quantidade {q} fora das faixas válidas para armazenamento da massa de bolo de chocolate."
            )
        ...
        logger.info(
            f"🕒 Duração calculada: {self.duracao} para {q}g de massa de bolo de chocolate."
        )
    ...
    def iniciar(self):
        ...
        if not sucesso:
            raise Exception(
                ...
                f"para {self.quantidade_produto}g de massa para bolo de chocolate."
            )
