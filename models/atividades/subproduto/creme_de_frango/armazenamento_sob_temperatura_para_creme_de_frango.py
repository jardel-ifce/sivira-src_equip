...
class ArmazenamentoSobTemperaturaParaCremeDeFrango(Atividade):
    ...
    def calcular_duracao(self):
        ...
        if 3000 <= q <= 20000:
            ...
            raise ValueError(
                ...
                "Armazenamento Sob Temperatura de Creme de Frango."
            )

        logger.info(
            f"🕒 Duração definida: {self.duracao} para {q}g de creme de frango."
        )
    ...
    def iniciar(self):
        ...
        logger.info(
            f"🚀 Armazenamento sob temperatura do creme de frango iniciado na {self.equipamento_alocado.nome} "
            f"de {self.inicio_real.strftime('%H:%M')} até {self.fim_real.strftime('%H:%M')}."
        )
        print(
